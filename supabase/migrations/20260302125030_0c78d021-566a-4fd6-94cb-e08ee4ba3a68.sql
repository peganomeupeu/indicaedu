...
AS $function$
  ...
  ORDER BY 
    (
      (COUNT(*) - COUNT(*) FILTER (WHERE r.status = 'inscrito') - COUNT(*) FILTER (WHERE r.status = 'qualificado') - COUNT(*) FILTER (WHERE r.status = 'nao_convertido') - COUNT(*) FILTER (WHERE r.status = 'nao_qualificado')) * 10 +
      COUNT(*) FILTER (WHERE r.status = 'qualificado') * 30 +
      COUNT(*) FILTER (WHERE r.status = 'inscrito') * 50 +
      COUNT(*) FILTER (WHERE r.status = 'nao_convertido') * 30 +
      COUNT(*) FILTER (WHERE r.status = 'nao_qualificado') * 10
    ) DESC,
    COUNT(*) DESC
$function$;
